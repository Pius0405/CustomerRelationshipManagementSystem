...
// Get total number of customers across all users
$cust_result = $conn->query("SELECT COUNT(*) AS count FROM customer");
$cust_count = $cust_result->fetch_assoc()["count"];

// Get total number of leads across all users
$lead_result = $conn->query("SELECT COUNT(*) AS count FROM lead");
...
$customerReminders = $conn->prepare("
    SELECT cr.date, cr.reminder, c.name, c.customer_id, u.name as user_name 
    FROM customerreminder cr
    JOIN customer c ON cr.customer_id = c.customer_id 
...
$leadReminders = $conn->prepare("
    SELECT lr.date, lr.reminder, l.name, l.lead_id, u.name as user_name
    FROM leadreminder lr
    JOIN lead l ON lr.lead_id = l.lead_id 
...
while($row = $customerResult->fetch_assoc()) {
    ...
    $reminders[$date][] = [
        ...
        'id' => $row['customer_id'],
        'user_name' => $row['user_name']  // Add sales rep name
    ];
}
            ...
            <div class="dashboard-left-column">
                <div class="left-column-container">
                    <div class="dashboard-block-container">
                        <div class="total-user-block" onclick="window.location.href='admin.php'">
                            <div class="people-icon-container">
                                <img class="people-icon" src="../general/dashboard-images/people-icon.png">
                            </div>
                            <p class="total-user-title">Total Staffs</p>
                            <?php echo "<p class='user-count'>" . $user_count . "</p>"; ?>
                        </div>

                        <div class="total-user-block" onclick="window.location.href='all-customers.php'">
                            <div class="people-icon-container">
                                <img class="people-icon" src="../general/dashboard-images/customer-icon.png">
                            </div>
                            <p class="total-user-title">Total Customers</p>
                            <?php echo "<p class='user-count'>" . $cust_count . "</p>"; ?>
                        </div>

                        <div class="total-user-block" onclick="window.location.href='all-leads.php'">
                            <div class="people-icon-container">
                                <img class="people-icon" src="../general/dashboard-images/lead-icon.png">
                            </div>
                            <p class="total-user-title">Total Leads</p>
